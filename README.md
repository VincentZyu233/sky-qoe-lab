...
| `/v1/outfits` | 十个槽位的全部服饰定义、32 位 ID、季节和衣柜标志喵 |
...
实机已验证六个端点可解析，并确认正常关闭后 `27891` 不再监听喵。
...
| `SkyQoELoader.exe` | 把当前菜单 DLL 内嵌为资源，在其他 Windows x64 电脑上一键校验、注入或安全重载喵 |
...
| PowerShell `Invoke-RestMethod` | 验证六个本机 HTTP 端点及关闭后的端口释放喵 |
...
| `SkyQoEMenu.dll` | 可直接由 CE Bridge 注入的 v0.4.0 菜单 DLL 喵 |
