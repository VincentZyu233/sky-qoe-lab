...
- `SkyQoEMenu.dll` v0.7.0 将外部 API 扩展到环境、实体、房间成员、TGCL 对象、聊天捕获与异步发送，并让 Loader 每秒展示这些状态；实机 `/health` 的五项就绪状态均为 `true` 喵。
...
## v0.7.0 Loader 实时状态面板

`SkyQoELoader.exe` 现在同时充当游戏外的中文实时状态面板，每秒向 `http://127.0.0.1:27891/v1/state` 发起一次只读请求喵。

窗口显示当前玩家有效性、Avatar 索引与状态位、XYZ 坐标、地图和扫描状态、TGCL 对象/属性数量、附近 Transform、Wax 点、循环烛火与全特效开关喵。

下方三个可滚动区域分别展示十个服饰槽位的 `internal_name` 与有效 ID、房间当前/最大人数和成员距离/坐标、聊天 Hook/队列/缓冲与捕获发送计数喵。

HTTP 请求和 JSON 解析全部在独立后台线程完成，Win32 消息线程只接收已经整理好的文本，因此 API 超时、返回坏 JSON、菜单重载或 Sky 退出都不会卡住 Loader 窗口喵。

请求使用短本机超时和 2 MiB 响应上限，缺字段、错误状态码与无效 UTF-8 都会变成中文连接状态；API 恢复后下一轮会自动重连，无需重启 Loader 喵。

“注入”和“重载菜单”仍在各自的后台动作线程运行；操作期间实时面板显示等待重连，动作结束后继续刷新，关闭 Loader 仍不会卸载游戏内菜单喵。

2026-07-24 实机验证：Loader 在 Sky PID `28464` 上显示 v0.7.0 API、人物坐标、`CandleSpace` 环境统计、十槽穿搭、房间成员与聊天就绪状态；连续刷新期间 Sky 与 Loader 均保持响应，两个自动功能均为关闭喵。

## v0.7.0 异步本机 HTTP API
...
2026-07-24 实机验证：v0.7.0 在 `CandleSpace` 返回 10,048 个对象、198 个类型、2,390 个符号、34,665 个属性、44 个来源和 13 个 Wax 生成器；连续两个聊天任务成功后再等待 20 秒，Sky 仍响应、发送失败为 0 且两个自动功能关闭喵。
...
控制导出 `SkyQoE_SetWaxLoopEnabled`、`SkyQoE_SetWaxLoopInterval`、`SkyQoE_SetLocalEffectLoopEnabled` 和 `SkyQoE_SetLocalEffectInterval` 继续供 CE Bridge 使用；HTTP v0.7.0 只开放受限聊天发送，不开放传送、换装或自动功能写接口喵。
...
| `SkyQoEMenu.dll` | 可直接由 CE Bridge 注入的 v0.7.0 菜单 DLL 喵 |
