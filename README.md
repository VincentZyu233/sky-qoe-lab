...
| `0x13F88C0` | `Game::AddChatMessage` | 游戏显示/接收聊天消息的入口，v0.6.0 在此捕获文本、来源和频道喵 |
| `0x10E1670` | `Game::OnKeyboardComplete` | UI 完成聊天输入后的原生提交入口，v0.6.0 只在游戏线程调用喵 |
...
- `SkyQoEMenu.dll` v0.6.0 将外部 API 扩展到环境、实体、房间成员、TGCL 对象、聊天捕获与异步发送；实机 `/health` 的五项就绪状态均为 `true` 喵。
...
任务 `succeeded` 表示原生 `Game::OnKeyboardComplete` 调用已经在游戏线程返回，不代表服务器一定接受、广播或送达，外部程序不能把它当送达回执喵。
...
| `Game::OnKeyboardComplete` | `0x10E1670` | UI 提交输入后的原生发送入口，异步任务在游戏线程调用喵 |
...
当前 UI 调用 `OnKeyboardComplete` 时使用 `(GameRoot, message, false, true)`；发送前还会重新校验 PE 构建、GameRoot vtable 和函数前导 16 字节喵。

`AddChatMessage` 当前按 `(Game*, text, SourceUuid*, channel, context)` 捕获；文本和来源均通过受检进程内读取复制到固定缓冲，读取失败只增加 `captureDropped` 喵。

2026-07-24 实机验证：v0.6.0 在 `CandleSpace` 返回 10,048 个对象、198 个类型、2,390 个符号、34,665 个属性、44 个来源和 13 个 Wax 生成器，聊天发送任务在游戏线程完成后 Sky 保持响应喵。
...
控制导出 `SkyQoE_SetWaxLoopEnabled`、`SkyQoE_SetWaxLoopInterval`、`SkyQoE_SetLocalEffectLoopEnabled` 和 `SkyQoE_SetLocalEffectInterval` 继续供 CE Bridge 使用；HTTP v0.6.0 只新增受限聊天发送，不开放传送、换装或自动功能写接口喵。
...
| `SkyQoEMenu.dll` | 可直接由 CE Bridge 注入的 v0.6.0 菜单 DLL 喵 |
...
25. 聊天发送不能从 HTTP 工作线程直接调用 `OnKeyboardComplete`；必须进入有界队列并由 `EmitterBarn::Update` 游戏线程消费，否则线程归属错误可能让游戏卡住或崩溃喵。
26. `MH_RemoveHook` 不能先于在途回调退出，否则回调可能跳回已经释放的 trampoline；卸载必须先 `MH_DisableHook`、等待 inflight 归零，再移除 Hook 喵。
27. 聊天发送任务的 `succeeded` 只表示原生提交调用返回，不是服务器送达回执；本地发送也不保证再次进入 `AddChatMessage` 捕获环喵。
